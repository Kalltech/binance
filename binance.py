...
def formatNumber(x):
 ...
 # returns lot info for a given token
def lotInfo(symbol, **kwargs):

    data = request("GET", "/api/v1/exchangeInfo", {})
    return data
    
    
# call at begining of any session to load min and max quantity data from server
def initializeLotInfo():
    raw = lotInfo('abc')['symbols']
    for key in raw:
        lots[key['baseAsset']] = key['filters'][1]
        #lots[key['baseAsset']]
    for lot in lots:
...
def numAfterPoint(x):
    # helper method for counting places after decimal point
    s = str(x)
    if not '.' in s:
        return 0
